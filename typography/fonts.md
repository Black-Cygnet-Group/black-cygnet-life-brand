...
Black Cygnet Life uses three fonts. **No font files are committed to this repo** — commercial font licences almost always forbid redistribution via public repositories. This document tells you where to obtain each one.
...
For internal marketing communication only. Use sparingly.

**Weights used:** Regular (400).

**Where to get it.** **Source and licence to be confirmed by the brand team.** The internal brand notes explicitly state `"Licenses: Pending??"`. See [`../OPEN-QUESTIONS.md`](../OPEN-QUESTIONS.md) item 3. Until this is resolved, do not assume the font is available — coordinate with the brand team to obtain licensed copies or substitute with a compatible alternative on a per-project basis.
